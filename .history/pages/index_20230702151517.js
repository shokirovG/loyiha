...
import {
  getAllCategories,
  getAllCategoriesChick,
  getAllCategoriesFood,
} from "@/services/api";
...
export default function Home() {
  ...
  const { meals, isLoading, seameals, SeaLoading, chickmeals, ChickLoading } =
  ...
  useEffect(() => {
    ...
    getAllCategoriesChick().then((data) => dispatch(ChickmealFetched(data)));
  }, []);
  // 1
  ...
  // 3
  const renderMealListChick = (chickmeals) => {
    if (chickmeals.length === 0) {
      return <h1>Meals not found!</h1>;
    }
    if (ChickLoading === "loading") {
      <Loader />;
    } else if (ChickLoading === "loadingError") {
      <Error />;
    }
    return <ChickFoodList chickmeals={chickmeals.slice(0, 6)} />;
  };
  //----------returning----------//
  return (
    <div className="App">
      ...
      <div className="container p-5">
        ...
        {renderMealListChick(chickmeals)}
        <Map />
        <Footer />
      </div>
    </div>
