...
const getAllCategoriesFood = async () => {
...
const getAllCategoriesDrinks = async () => {
  const res = await fetch(`http://localhost:3000/api/drinksapi`).then(
    (responce) => responce.json()
  );
  return res;
};

const getAllCategoriesChick = async () => {
  const res = await fetch(`http://localhost:3000/api/chickenapi`).then(
    (responce) => responce.json()
  );
  return res;
};
